...
2.  DB연동
    ...
    5.  MVC 패턴에 맞춰 각 기능별로 패키지(폴더) 생성
        ...
        - controller, entity, repository, sevice
...
2. DB연동 개발 계속
   ...
   3. @(Annotation) 추가
      ...
      - @Service : 서비스 모듈을 지정
      - @RequiredArgsConstructor : final등의 멤버변수를 파라미터로 생(Lombok)
      - @AllArgsConstructor : 클래스 멤버변수를 사용해서 생성자를 만들어 주는것(Lombok)
...
1.  DB연동 개발
    ...
    2.  Spring Boot Validation 기능 추가 : 입력 검증
        ...
        ```
            ...
            - Annotation으로 검증 수행 : @Size, @NotNull, @NotEmpty, @Past(과거날짜만 가능), @Future(미래날짜만 가능), @FutureOrPresent(미래또는 오늘날짜만), @Pattern(정규식 패턴)
    ...
    6.  board_detail.html에 댓글 입력 검증 태그 추가
        - ReplyController의 PostMapping 매서드에 ReplyForm을 파라미터로 추가
        - BoardController의 GetMapping 매서드에 ReplyForm을 @Valid 파라미터로 추가
...
4.  Paging : 대량 데이터 로드시 속도 개선
    ...
    - board_list.html에 페이징 컨트롤 추가
        ...
        <img src="./image/day8-01.png" width="600">
...
1. 게시판 작업
   ...
   1. Paging 구현 계속
      ...
      - 맨 첫페이지, 맨 마지막페이지 버튼 추가

   2. 게시글 최신 글부터 나오도록 정렬

      1. BoardService getBoardList() 매서드에 정렬로직 추가

   3. 게시글 갯수만큼 번호가 나오도록 수정

      1. 현재는 각 페이지마다 1~10까지 반복
      2. 게시물 번호 = 전체 게시물개수 - (현재페이지번호 \* 10[페이지당 게시글 수]) - 페이지 당 인덱스
      3. board_list.html에 `<td th:text="${index.count}"></td>`을 수정
   ...
   4. Bootstrap 배지로 각 게시글마다 댓글개수 표시
    ...
    <img src="./image/day9-01.png" width="500">

2. Spring Boot Security : 회원가입, 로그인등을 손쉽게 개발하도록 도와주는 의존성 라이브러리
   ...
   2. 로그인 화면 및 H2 DB 사용불가
      - 기본사용자 : user
      - 패스워드 : SpringBoot 로그에 표시(서버 실행 할 때마다 변경)
   3. 스프링 시큐리티 설정
